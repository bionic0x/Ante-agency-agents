...
The candidate roster comes from the existing runbook and canonical catalog. Only
...
budget. Cross-scope inputs are rejected unless they are explicitly shared.
A host must separately verify whether a `shared` classification is defensible.
...
| `rebind_claims` | Task ID, exact set of input claims with reviewed revisions, and reason |
| `dissent` | Objection, evidence references and residual-risk owner; preserved in history |
| `close` | Evidence and closure record; all planned work must be complete and current |
