...
A tiny isolated repo proves incremental behavior without mutating the checkout or
paying the real 487-agent corpus cost. The strict conversion suite remains the
byte-identity authority for all production adapters.
"""
...
def make_fixture(parent: Path) -> Path:
    ...
    for name in ("convert.sh", "convert-engine.sh", "lib.sh", "registry.py", "integration-state.py"):
...
class IncrementalConversionTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.engine = self.repo / "scripts" / "convert-engine.sh"
    ...
    def sync(self, tool: str) -> subprocess.CompletedProcess[str]:
        return run(self.repo, "bash", str(self.convert), "--tool", tool)
    ...
    def test_rename_delete_corruption_and_orphan_pruning(self) -> None:
        ...
        renamed_source = self.repo / "testing" / "testing-renamed-alpha.md"
        self.alpha.rename(renamed_source)
        self.alpha = renamed_source
        path_renamed = self.sync("codex")
        self.assertIn("incremental rebuild (1 changed/new, 1 removed)", path_renamed.stdout)
        self.assertTrue(new_output.is_file())
    ...
    def test_renderer_change_and_roster_tool_fall_back_to_full_rebuild(self) -> None:
        self.sync("codex")
        with self.engine.open("a", encoding="utf-8") as handle:
        ...
        self.assertIn(
            "Roster change.",
            (self.repo / "integrations" / "aider" / "CONVENTIONS.md").read_text(encoding="utf-8"),
        )

    def test_explicit_out_bypasses_state_wrapper(self) -> None:
        out = self.repo / "scratch-output"
        completed = run(
            self.repo,
            "bash",
            str(self.convert),
        ...
        self.assertIn("Converted 2 agents for codex", completed.stdout)
        self.assertTrue((out / "codex" / "agents" / "alpha-agent.toml").is_file())
        self.assertFalse(self.state_file("codex").exists())

    def test_duplicate_output_ownership_is_rejected(self) -> None:
        duplicate = self.repo / "testing" / "testing-duplicate.md"
        duplicate.write_text(AGENT_TEMPLATE.format(name="Alpha Agent"), encoding="utf-8")
        checked = self.check_state("codex")
        self.assertNotEqual(0, checked.returncode)
        self.assertIn("ambiguous output ownership", checked.stderr)
